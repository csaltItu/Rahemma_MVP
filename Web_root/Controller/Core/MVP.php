...
if(isset($_REQUEST["calltype"])) {	// This is not incoming call as $calltype is set 
	...
	$userid 	 = $_REQUEST["phno"];
 	...
 	$app 		 = $_REQUEST["app"];
 	$From 		 = $_REQUEST["From"];
 	$MsgLanguage 		 = $_REQUEST["MsgLanguage"];
 	$FollowupReqID 		 = $_REQUEST["FollowupReqID"];
	...
	$fh = createLogFile($callid);	// commented out for cloud deployment
 	...
 	if(isset($_REQUEST["error"])){
		...
		writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L0", "Call Failed, error:".$error);
 		switch ($error) {
 			case 'USER_BUSY':
 			case 'CALL_REJECTED':
 				busyFCN($callid);
	...
	if($channel == "WateenE1"){
		...
		// writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L1", $ocallid. " " . gettype($ocallid));
	    if($FreeSwitch == true){
	    	if (strcmp($FollowupReqID, "500") == 0) {
	    		StartUpFn();
	    		MVP_Delivery_ANS($recIDtoPlay, $effectno, $MsgLanguage);
	    	} else if($calltype == "REM-ROBO" || $calltype == "REM-ROBO-FWD"){
	    		StartUpFn();
	    		ReahemaRoboCall();
	    		...
	    		|| $calltype == "REM-CMB-CB" || $calltype == "REM-CMB-RK"){
	    		StartUpFn();
	    		RahemmaMVP();
	    		...
	    		$calltype == "REM-DEL-CF" || $calltype == "REM-DEL-PF" || $calltype == "REM-DEL-LR" || $calltype == "REM-DEL-S"
	    		 || $calltype == "REM-DEL-CB" || $calltype == "REM-DEL-RK"){
	    		StartUpFn();
	    		MVP_Delivery();
	    	}else{
		    	Prehangup();
	...
	if(strpos($Pollyid, $ReMCMBCF) !== FALSE){
		...
		$app 		 = 'Raah-e-Maa-Robo';
	}else if(strpos($Pollyid, $ReMCMBCB) !== FALSE){
		$testcall 	 = "TRUE";
		$requestType = "REM-CMB-CB";
		$calltype 	 = 'REM-MC-CB';
		$app 		 = 'Raah-e-Maa-Cable';
	}else if(strpos($Pollyid, $ReMCMBRK) !== FALSE){
		$testcall 	 = "TRUE";
		$requestType = "REM-CMB-RK";
		$calltype 	 = 'REM-MC-RK';
		$app 		 = 'Raah-e-Maa-RickShaa';
	}else{
		$requestType = "";
		$testcall = "FALSE";
	...
	if (!in_array($userid, $blocked_Users)) {
		...
		if(searchCalls($userid)>1 || $userid < '1000'){	// If its a missed call then reject() will generate 2 retries from the Cisco equipment. This is to ignore those. OR If its a self loop call... ignore it. &&$$** Added < '1000' in place of == '04238333111'
			...
			updateCallStatus($callid, "Complete");
		...
		writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L1", "Not making a request because User is Blocked in MVP");
		updateCallStatus($callid, $status);
		writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L1", "Call Complete. Now exiting.");
		markCallEndTime($callid);
	}
...
function isThisCallActive(){
	...
	writeToLog($callid, $fh, "L2", "Is the current call active? ".$retVal.". Hanging up if the call is not active.");

	// if (strpos($retVal, "Nothing")!== FALSE) {
	// 	$cmd = "api lua isActive.lua ".$uuid;
	// 	$response = event_socket_request($fp, $cmd);
	// 	$retVal =  trim($response);
	// }
	// writeToLog($callid, $fh, "L2", "Is the current call active? ".$retVal.". Hanging up if the call is not active.");
	// $retVal = str_replace("Content-Type: api/response", "", $retVal);
	// writeToLog($callid, $fh, "L2", "Is the current call active- modified value  ".$retVal);
...
function rejectCall($app) {
	...
	if($FreeSwitch == "false") {
	    ...
	    $cmd = "api lua rejectMVP.lua ".$uuid;//first character is a null (0)
		$response = event_socket_request($fp, $cmd);
	}
	Prehangup();
}
...
function RahemmaMVP() {
	...
	// sayInt($promptToplay);
	// $promptToplay = $MVP_prompts."PublicAnswer2.3.wav ". $MVP_prompts."PublicAnswer2.3.wav ".$MVP_prompts."Press2.wav ". $MVP_prompts."PublicAnswer2.2.wav ".$MVP_prompts."Press3.wav ". $MVP_prompts."PublicAnswer2.4.wav";
	// sayInt($promptToplay);
	// sayInt('D:/xampp/htdocs/MVP/Story/11.wav');
	sayInt($MVP_prompts."MainPrompt1.wav");
	// sayInt($MVP_prompts."Mainpromptwq.wav");
	sayInt($MVP_prompts."Disclaimer1.wav");
	sayInt($MVP_prompts."Disclaimer2.wav");

	$loop = true;
// sayInt($MVP_prompts."Mainpromptwq.wav");
	while ($loop) {

		$prompt =   $MVP_prompts."Mainpromptwq.wav";
...
function ReahemaRoboCall() {
	...
	$roboFile = "D:/xampp/htdocs/MVP/roboNumber.txt";
	$roboTextFile  = fopen($roboFile, 'r');
	$number = fgets($roboTextFile);
	fclose($roboFile);

	sayint($MVP_prompts ."Mushi2.wav");
	sayint($MVP_prompts . "RC" . $number .".wav");
	// sayint($MVP_prompts ."MushiBoy1.wav ".$MVP_prompts ."sil250.wav " .$MVP_prompts ."MushiBoy2.wav");
	// $promptToplay = $MVP_prompts . "RC6.wav";

	$promptToplay = $MVP_prompts . "RC6.wav " .$MVP_prompts . "RC7.wav ". $MVP_prompts . "Press2.wav " . $MVP_prompts . "RC9.wav" ;
	
	writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L1", $number . " " . $promptToplay);

	$roboTextFile  = fopen($roboFile, 'w');
	if ($number == 5) {
		fwrite($roboTextFile, 1);
	} else {
		fwrite($roboTextFile, $number + 1);
	}
	fclose($roboFile);
	...
	while ($loop) {
		...
		writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L1", __FUNCTION__ . " prompt: ".$promptToplay);
...
function askQuestion() {
	...
	sayInt($MVP_prompts."Disclaimer3.wav");
	...
	while ($question_id && $loop) {
		...
		if ($rerecord) {
			recordAudio($MVP_prompts."RecordQ1.wav", array(
		       "beep"=>true,
		       "timeout"=>300,
		       "bargein" => false,
		       "silenceTimeout"=>4,
	    ...
	    $result = gatherInput( $MVP_prompts."RecordQ3.wav", array(
									"choices" => "[1 DIGITS]",
									"mode" => 'dtmf',
									"bargein" => true,
									"repeat" => 100,
    	...
    	if ($result->value == "1") {
    		...
    		sayInt($MVP_prompts."RecordQ4.wav");
	    	$loop = false;
		    while (true) {
			    $result2 = gatherInput( $MVP_prompts."Disclaimer4.wav", array(
											"choices" => "[1 DIGITS]",
											"mode" => 'dtmf',
											"bargein" => true,
											"repeat" => 100,
...
function listenOwnQuestion() {
	...
	$new_questions = getUserQuestions(0);
	...
	while($loop) {
		...
		if ($qid >= sizeof($questions)) {
			if ($ex_questions && $new_questions) {
				$questions = $new_questions;
				sayInt($MVP_prompts."Nomoreoldqas.wav");
				$qid = 0;
				$ex_questions = false;
			} else {
	...
	*/
			if ($question_action == "3") {
				...
				setLog(getActionID("listen_q"), $question_id);
				sayInt($MVP_prompts."Yourquestion.wav ".$MVP_recordings."Q".$question_id.".wav");
				sayInt($MVP_prompts."Irrelevant.wav ");
			}else if ($question_action == "5") {
				setLog(getActionID("listen_q"), $question_id);
				sayInt($MVP_prompts."Yourquestion.wav ".$MVP_recordings."Q".$question_id.".wav ");
				sayInt($MVP_prompts."Inappropriate.wav ");
			}else if ($question_action == "6") {
				setLog(getActionID("listen_q"), $question_id);
				sayInt($MVP_prompts."Yourquestion.wav ".$MVP_recordings."Q".$question_id.".wav ");
				sayInt($MVP_prompts."Inaudible.wav ");
			}else if ($question_action == "7") {
				setLog(getActionID("listen_q"), $question_id);
				sayInt($MVP_prompts."Yourquestion.wav ".$MVP_recordings."Q".$question_id.".wav ");
				sayInt($MVP_prompts."Irresponsive.wav ");
				...
				sayInt($MVP_prompts."Yourquestion.wav ".$MVP_recordings."Q".$question_id.".wav");
				sayInt($MVP_prompts."Jawaab.wav ".$MVP_recordings."FAQ".$faq.".wav ". $MVP_prompts. "Postanswer.wav");
				if (Vote($question_id, "question") == 1) {
					$loop = true;
					$play = true;
					continue;
				}
				...
				sayInt($MVP_prompts."Yourquestion.wav ".$MVP_recordings."Q".$question_id.".wav");
				sayInt($MVP_prompts."Jawaab.wav ".$MVP_recordings."A".$answer_id.".wav ". $MVP_prompts. "Postanswer.wav");
				if (Vote($question_id, "question") == 1) {
					$loop = true;
					$play = true;
					continue;
...
function allQuestions() {
	...
	$new_questions = getQuestions(getActionID("listen_q"), 0, 0);
	$questions = $new_questions;
    ...
    while ($loop) 	{
		if ($qid >= sizeof($questions)) {
			if ($ex_questions && $new_questions) {
				...
				sayInt($MVP_prompts."NoMorePublic.wav");
				break;
			}
		}
		...
		if(is_null($question_id)){
			...
			if(!is_null($answer_id) && $play) {
				sayInt($MVP_prompts."Sawaal.wav ".$MVP_recordings."Q".$question_id.".wav");
					sayInt($MVP_prompts."Jawaab.wav ".$MVP_recordings."A".$answer_id.".wav");
		 		$play = false;
			} else if (!is_null($faq) && $play) {
				// sayInt($MVP_prompts."Sawaal.wav ".$MVP_recordings."Q".$question_id.".wav");
				// 	sayInt($MVP_prompts."Jawaab.wav ".$MVP_recordings."FAQ".$faq.".wav ". $MVP_prompts. "Postanswer.wav");		W	
		 		$play = false;
			}
...
function listenStories() {
	...
	$new_files = MVP_GetFile($userid, "Story", 0);
	$files = $new_files;
	// writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L1", "SIZE:" . sizeof($files));

	$ex_files = MVP_GetFile($userid, "Story", 1);

	// writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L1", "SIZE:". sizeof($ex_files));

	if (sizeof($ex_files) > 0) {

		$result = gatherInput( $MVP_prompts."Return2.wav", array(
				"choices" => "[1 DIGITS]",
				"mode" => 'dtmf',
				"bargein" => true,
				"repeat" => 100,
	...
	sayInt($MVP_prompts."Storydisclaimer1.wav");
	...
	while (true) {
		...
		if ($count >= sizeof($files)) {
			if ($ex_files && $new_files) {
				$files = $new_files;
				sayInt($MVP_prompts."Nomoreoldstories.wav");
				$count = 0;
				$ex_files = false;
			} else  {
				sayInt($MVP_prompts."Endofstories.wav");
				break;
			}
		}
				...
				if ($dreq) {
					if ($userid == 3566) {
						...
						if ($type == "RoboCall") {
							...
							$delType = "REM-DEL-CF";
						} else 	if ($calltype == "REM-CMB-RK"|| $calltype == "REM-DEL-RK") {
							$delType = "REM-DEL-RK";
						} else 	if ($calltype == "REM-CMB-CB"|| $calltype == "REM-DEL-CB") {
							$delType = "REM-DEL-CB";
						}
...
function MVP_Record($type, $record_id){
	...
	if($type == 'request'){
		...
		$maxtime = 10;
		$prompt  = $MVP_prompts."Forward11.wav ";
	}else if($type == 'story'){
		$maxtime = 120;
		sayInt($MVP_prompts."Apnistory.wav");
		sayInt($MVP_prompts."Storydisclaimer2.wav");
    ...
    while ($loop) {
		...
		$record_return = recordAudio($prompt,  
			array(
		       "beep"			=>	true,
		       "timeout"		=>	300,
		       "bargein"		=>	false,
		...
		if($type == "request"){
	   		...
	   		global $callid;
			$Name = $MVP_recordings."UserNames/".getFilePath($callid.".wav", "TRUE")."UserName-".$callid.".wav";
	   		sayInt($Name);
	   		global $NameRecorded;
			$NameRecorded = true;
...
function MVP_GetFile($uid, $type, $attempted){
	...
	$result = doCurl($MVP_scripts."get_file_id.php?uid=".$userid."&type=".$type."&call_id=".$callid."&attempted=".$attempted);
...
function MVP_Delivery($del_id){
	...
	if ($params = MVP_GetDeliveryParams($del_id)) {
		...
		$type = $params["type"];
		$faq = $params["faq"];
		$tCallID = $params['call_id'];
		$answer_id = $params["answer_id"];
		$Name = $MVP_recordings."UserNames/".getFilePath($tCallID.".wav", "TRUE")."UserName-".$tCallID.".wav";
		...
		// sayInt($Polly_prompts_dir. "sil1500.wav");
        // sayInt('D:/xampp/htdocs/MVP/Recs/U144490.wav');
        // sayInt("D:/xampp/htdocs/MVP/Recs/U1592.wav");
		...
		while($loop){
			...
			if ($type == "Story") {
				...
				if ($faq == "0") {
					sayInt($MVP_prompts."Sawaal.wav ".$MVP_recordings."Q".$id.".wav ");
					sayInt($MVP_prompts."Jawaab.wav ".$MVP_recordings."A".$answer_id.".wav");
				} else {
					sayInt($MVP_prompts."Sawaal.wav ".$MVP_recordings."Q".$id.".wav ");
					sayInt($MVP_prompts."Jawaab.wav ".$MVP_recordings."FAQ".$faq.".wav");
				}
			}
			
			$result = gatherInput($MVP_prompts."Forwardedoptions.wav ", array(
					"choices" => "[1 DIGITS]", 
					"mode" => 'dtmf',
					"bargein" => false,
					"repeat" => 100,
...
function MVP_Delivery_ANS($question_id, $answer_id,$type){
	...
	global $MVP_prompts, $MVP_recordings, $MVP_Story_Recordings;
		...
		while($loop){
			...
			$result = gatherInput($MVP_prompts."RC7b.wav", array(
					"choices" => "[1 DIGITS]", 
					"mode" => 'dtmf',
					"bargein" => false,
					"repeat" => 100,
			...
			if($result->value == "1") {
				...
				do {
					...
					if ($answer_id != -1) {
						...
						sayInt($MVP_prompts."Jawaab.wav ".$MVP_prompts.$type.".wav");
						break;
					}		
				} while (Vote($question_id, "question") == 1); 
				setLog(getActionID("listen_q"), $question_id);
				RahemmaMVP();			
			} else {
				break;
...
function Funstuff() {
	...
	addFunctionLog(__FUNCTION__, 0);
	global $userid;
	writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L1", " $userid funstuff main");
	writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L1", " $userid funstuff is on");
	global $MVP_prompts;
	global $MVP_recordings;
	global $MVP_scripts;
	global $funquestionmapping;

	// $loop = true;
	$loop = true;
	$add = 0;
	$prompt =   $MVP_prompts."PersonalityP1.wav ";
	// $prompt = $prompt.$MVP_prompts . "MoreQuizzes.wav ";
	$prompt = $prompt.$MVP_prompts . "RepeatOptions.wav ".$MVP_prompts . "Press7.wav " . $MVP_prompts . "Goback.wav " .$MVP_prompts . "Press0.wav";
	while ($loop) {
		// writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L1", __FUNCTION__ . " prompt: ".$prompt);
		$result = gatherInput($prompt, array(
						"choices" => "[1 DIGITS]",
		...
		if ($result->value >= 1 && $result->value <= 6 ) {
			$question = $result->value + $add;
			$question = $funquestionmapping[$question];
			addFunctionLog($question, 0);
			$quesstionAnswer = array();
			$questionCounter = 1;
			while ( true ) {
				$loop2 = true;

				addFunctionLog($question."Q".$questionCounter, 0);
				while ($loop2) {
					sayInt($MVP_prompts.$question."Q".$questionCounter.".wav");
					
					$result2 = gatherInput($MVP_prompts.$question."Q".$questionCounter."option1.wav ".$MVP_prompts.$question."Q".$questionCounter."option2.wav ".$MVP_prompts.$question."Q".$questionCounter."option3.wav ".$MVP_prompts.$question."Q".$questionCounter."option4.wav", array(
											"choices" => "[1 DIGITS]",
											"mode" => 'dtmf',
											"bargein" => false,
											"repeat" => 2,
					...
					if ($result2->value >= 1 && $result2->value <= 4 ) {
						array_push($quesstionAnswer, $result2->value);
						addFunctionLog($question."Q".$questionCounter, 1);
						$loop2 = false;
					} else {
						sayInt($MVP_prompts."Wrongbutton.wav");
						$loop2 = true;
				...
				if ($questionCounter > 4) {
					$quesstionAnswervalues = array_count_values($quesstionAnswer);
					arsort($quesstionAnswervalues);
					$answer = key($quesstionAnswervalues);
					sayint($MVP_prompts.$question."result.wav");
					sayint($MVP_prompts .$question. "A".$answer.".wav");
					addFunctionLog($question, 1);
					break;		
				} else {
					sayint($MVP_prompts."AglaSawaal.wav");
				}	
		...
		// 	$add += 6;
		// 	if ($add < 6) {
		// 		$prompt =   $MVP_prompts."PersonalityP2.wav ";
		// 	} else if ($add < 12) { {
		// 		$prompt =   $MVP_prompts."PersonalityP3.wav ";
		// 	} else {
		// 		$prompt =   $MVP_prompts."PersonalityP1.wav ";
		// 		$add = 0;
		// 	}

		// 	$prompt = $prompt.$MVP_prompts . "MoreQuizzes.wav ".$MVP_prompts . "RepeatOptions.wav " . $MVP_prompts . "Goback.wav " .$MVP_prompts . "Press0.wav";
...
function setLog($action, $file_id) {
	...
	$result = doCurl($MVP_scripts."API/api/add/log?user_id=".$userid."&action_id=".$action."&call_id=".$callid."&file_id=".$file_id);
...
function listen_comments($file_id) {
	...
	if ($comments_result["len"] > 0) {
		sayInt($MVP_prompts."Firstcomment.wav");
		$back = false;
		for ($i=0; $i < $comments_result["len"] && !$back; $i++) { 
			...
			while (true) {
	 			$result = gatherInput($MVP_prompts."Commentoptions3.wav", array(
						"choices" => "[1 DIGITS]",
						"mode" => 'dtmf',
						"bargein" => false,
						"repeat" => 100,
				...
				if($result->value == "3") { 
					...
					if ($i < $comments_result["len"] - 1 ) {
						...
						sayInt($MVP_prompts."Nomorecomments.wav");	
					}
					break;					
				} else if($result->value == "3") { 
					$back = true; 
...
function vote($file_id, $type) {
	...
	if (getuserResponse($file_id, $type) == false) {
		...
		$promptToplay = $MVP_prompts."Storyoptions1.1.wav ". $MVP_prompts."Press1.wav ". $MVP_prompts."Storyoptions1.3.wav " . $MVP_prompts."Press2.wav ". $MVP_prompts."Storyoptions1.2.wav ".$MVP_prompts."Press3.wav ". $MVP_prompts."Storyoptions1.4.wav " . $MVP_prompts."Press4.wav ";

		if ($userorder % 2 == 0) {
		$promptToplay = $MVP_prompts."Storyoptions1.1.wav ". $MVP_prompts."Press1.wav ". $MVP_prompts."Storyoptions1.2.wav " . $MVP_prompts."Press2.wav ". $MVP_prompts."Storyoptions1.3.wav ".$MVP_prompts."Press3.wav ". $MVP_prompts."Storyoptions1.4.wav " . $MVP_prompts."Press4.wav ";
		}
...
function event_socket_request($fp, $cmd) {
    ...
    if ($fp) {
	// writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L0", __FUNCTION__ . " entered " . $cmd);
        ...
        while (!feof($fp)) {
           ...
           if ($contentlength === 0) { //if contentlenght is already don't process again
               if (strlen(trim($buffer)) > 0) { //run only if buffer has content
                   ...
                   if (strcmp($temparray[0] , "Content-Length")==0) {
		...
		// $response = str_replace("Content-Type: api/response", "", $response);
		// writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L0", __FUNCTION__ . " is returning: $response");
        return $response;
    }
    else {
      echo "no handle";
...
function invalid($onBadCoice)
{
	...
	{
		return $MVP_prompts."Wrongbutton.wav";
	}
}
...
function gatherTnputFreeSwitch($toBeSaid,$invalidFS,$mindigitsFS,$maxdigitsFS,$maxattemptsFS,$timeoutFS,$bargein,$termFS,$validInput,$onTimeOutFS,$interdigitTimeout)
{
	...
	*/
	...
	while($repeat == "TRUE"){
		...
		$response = event_socket_request($fp, $cmd);
		// global $logFiletest3;
		// $q = fopen($logFiletest3, "a+");
		// fwrite($q,$response." reeesponse .\n");
	...
	//	writeToLog($GLOBALS['callid'], $GLOBALS['fh'], "L2", "The ask() in " . __FUNCTION__ . " is now complete.");
		// fwrite($q,$output->value . " reeesponse1 .\n");
		// fclose($q);
...
function recordAudio($toBeSaid, $params){
	       	...
	       	if( strpos($recordURI, 'process_feedback') !== false )
	        ...
	        else if( strpos($recordURI, 'save_recording') !== false )
	        	{
			        ...
			        if($type == "request"){ // QU-5972-3.wav
						...
						global $MVP_recordings, $callid;
							$filepathFS= $MVP_recordings . "UserNames/" ;//"C:/xampp/htdocs/wa/Praat/UserNames/";
	...
	doCurl($MVP_scripts."endpoint_recording.php?fname=".$filepathFS1);
...
function sayInt($toBeSaid){
	...
	if(strcmp(isThisCallActive(), "true")== 0)
	{
		$choices = "[1 DIGITS], *, #" ;
		$mode = 'dtmf';
		$repeatMode = 0;
