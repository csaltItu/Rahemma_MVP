...
function getDel(){
	...
	$sql = 'SELECT type, file_id, user_id,content_type,call_id FROM forward WHERE forward_id = "'.$id.'"';
	...
	if ($result = $conn->query($sql)) {
		if($row = $result->fetch_row()) {
			...
			$faq = -1;
			if ($row[0] == 'Question') {
				$sql = 'SELECT answer_id,faq FROM question_answer WHERE question_id= "'.$file_id.'"';
			...
			return array('error' => false, 'type' => $type, 'file_id' => $file_id, 'fuid' => $fuid, 'answer_id' => $answer_id, 'faq' => $faq, 'call_id' => $call_id);
